...
def print_element(element):
    print("Elemento antes de enviar a BigQuery:", element)
    return element


""" Dataflow Process """
TU_UMBRAL_DE_DISTANCIA = 0.1  # en kilómetros 
...
def run():
    ...
    # Datos de coches
        coches = (
            ...
            #| "Extraer Datos de Coches" >> beam.Map(lambda x: x[1])  # Aquí extraes solo los datos del coche
            
            #| "Print Element" >> beam.Map(print_element)  # Agrega esta línea para imprimir cada elemento
        )
        ...
        personas = (
            ...
            #| "Extraer Datos de personas" >> beam.Map(lambda x: x[1]) 
        )
         
        grouped_data= (
            {'coches': coches, 'personas': personas}
            | "Group by dummy key" >> beam.CoGroupByKey()
            )
        matches = (
            grouped_data
            | "Buscar coincidencias" >> beam.ParDo(FindMatchFn())
        )
       
       # matches_ready_for_bq = (
       # matches
        #    | "Preparar coincidencias para BigQuery" >> beam.Map(lambda match: match[1])
         #   | "Escribir coincidencias en BigQuery" >> beam.io.WriteToBigQuery(
          #  table="civic-summer-413119:midataset_coches.matches",
           # schema='coche_id:STRING, persona_id:STRING, distance:FLOAT',
